...
export var languagePluginLoader = new Promise((resolve, reject) => {
    ...
    // way.
    var baseURL = window.self.languagePluginUrl || 'https://pyodide.cdn.iodide.io/';
    ...
    function loadScript(url, onload, onerror) {
      if (window.self.document) { // browser
      ...
      } else if (window.self.importScripts) { // webworker
        try {
          window.self.importScripts(url);
          onload();
        } catch {
          onerror();
        }
    ...
    let _loadPackage = (names, messageCallback) => {
      ...
      let packages = window.self.pyodide._module.packages.dependencies;
      let loadedPackages = window.self.pyodide.loadedPackages;
      ...
      window.self.pyodide._module.locateFile = (path) => {
      ...
      let promise = new Promise((resolve, reject) => {
        ...
        window.self.pyodide._module.monitorRunDependencies = () => {
          packageCounter--;
          if (packageCounter === 0) {
            for (let pkg in toLoad) {
              window.self.pyodide.loadedPackages[pkg] = toLoad[pkg];
            }
            delete window.self.pyodide._module.monitorRunDependencies;
            window.self.removeEventListener('error', windowErrorHandler);
            if (!isFirefox) {
        ...
        window.self.addEventListener('error', windowErrorHandler);
        ...
        window.self.pyodide.runPython('import importlib as _importlib\n' +
    ...
    var postRunPromise = new Promise((resolve, reject) => {
      Module.postRun = () => {
        delete window.self.Module;
        fetch(`${baseURL}packages.json`)
            .then((response) => response.json())
            .then((json) => {
              fixRecursionLimit(window.self.pyodide);
              window.self.pyodide.globals =
                  window.self.pyodide.runPython('import sys\nsys.modules["__main__"]');
              window.self.pyodide = makePublicAPI(window.self.pyodide, PUBLIC_API);
              window.self.pyodide._module.packages = json;
              if (window.self.iodide !== undefined) {
                // Perform some completions immediately so there isn't a delay on
                // the first call to autocomplete
                window.self.pyodide.runPython('import pyodide');
                window.self.pyodide.runPython('pyodide.get_completions("")');
              }
              resolve();
            });
      };
    ...
    loadScript(data_script_src, () => {
      ...
      loadScript(scriptSrc, () => {
        ...
        window.self.pyodide = pyodide(Module);
        window.self.pyodide.loadedPackages = new Array();
        window.self.pyodide.loadPackage = loadPackage;
      }, () => {});
    }, () => {});
    ...
    if (window.self.iodide !== undefined) {
      ...
      window.self.iodide.addOutputRenderer({
        shouldRender : (val) => {
